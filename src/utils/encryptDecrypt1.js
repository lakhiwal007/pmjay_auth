...
export class AESUtil {
	...
	encrypt(passPhrase, plainText) {
		...
		let salt = CryptoJS.lib.WordArray.random(this.keySize / 8).toString(
			CryptoJS.enc.Hex
		);
		let ciphertext = this.encryptWithIvSalt(
			salt,
			iv,
			passPhrase,
			plainText
		);
		// console.log("salt", salt, "iv", iv, "ciphertext", ciphertext);
